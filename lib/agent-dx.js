...
function applyCalendarRoles(action, result, domainConfig) {
  const roleDefs = getRoleDefs(domainConfig);
  const hasRoleDefs = roleDefs.length > 0;

  if (action === "events" || action === "search") {
    const events = Array.isArray(result?.events)
      ? result.events.map((event) => ({ ...event, calendarRole: getRoleKey(event.calendar, domainConfig) }))
      : result?.events;
    const updated = { ...result, events };
    return hasRoleDefs ? { calendarRoles: roleDefs, ...updated } : updated;
  }
  ...
  if (action === "get") {
    ...
    return {
      ...result,
      event: { ...result.event, calendarRole: getRoleKey(result.event.calendar, domainConfig) },
    };
  }

  if (action === "list") {
    return hasRoleDefs ? { calendarRoles: roleDefs, ...result } : result;
  }
...
export function withAgentDX(toolName, handler) {
  return async function agentDXHandler(args, runCLI, ...rest) {
    ...
    if (args.dryRun) {
      ...
      const withRoles = toolName === "calendar" && domainConfig
        ? applyCalendarRoles(args.action, acFiltered, domainConfig)
        : acFiltered;
      const filtered = applyFieldSelection(withRoles, args.fields);
    ...
    // 5.5. Calendar role injection
    const withRoles = toolName === "calendar" && domainConfig
      ? applyCalendarRoles(args.action, acFiltered, domainConfig)
      : acFiltered;

    // 6. Field selection — post-filter response
    return applyFieldSelection(withRoles, args.fields);
  };
}
