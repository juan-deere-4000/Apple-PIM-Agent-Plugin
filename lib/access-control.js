...
function validateDomain(domain, name, filePath) {
  ...
  const validKeys = new Set(["mode", "allow", "block", "readOnly", "default", "roles"]);
  ...
  if (domain.roles !== undefined) {
    if (typeof domain.roles !== "object" || domain.roles === null || Array.isArray(domain.roles)) {
      throw new Error(`access-control: "${name}.roles" must be an object (${filePath})`);
    }
    for (const [key, value] of Object.entries(domain.roles)) {
      if (typeof value !== "string") {
        throw new Error(`access-control: "${name}.roles.${key}" must be a string (${filePath})`);
      }
      if (!value.includes(":")) {
        throw new Error(`access-control: "${name}.roles.${key}" must contain a colon (format: "roleKey: description") (${filePath})`);
      }
    }
  }
...
/**
 * Get the role key for a calendar name from the domain config.
 * Returns "commitment" if no role is configured for the calendar.
 *
 * @param {string} calendarName
 * @param {object} domainConfig
 * @returns {string}
 */
export function getRoleKey(calendarName, domainConfig) {
  if (!domainConfig?.roles || !calendarName) return "commitment";
  const roleDef = domainConfig.roles[calendarName];
  if (!roleDef) return "commitment";
  const colonIdx = roleDef.indexOf(":");
  return colonIdx >= 0 ? roleDef.slice(0, colonIdx).trim() : "commitment";
}

/**
 * Get the list of unique role definitions from the domain config.
 * Returns an array of { role, description } objects for use in response headers.
 * Deduplicates by role key (first occurrence wins).
 *
 * @param {object} domainConfig
 * @returns {{ role: string, description: string }[]}
 */
export function getRoleDefs(domainConfig) {
  if (!domainConfig?.roles) return [];
  const seen = new Map();
  for (const value of Object.values(domainConfig.roles)) {
    const colonIdx = value.indexOf(":");
    const key = colonIdx >= 0 ? value.slice(0, colonIdx).trim() : value.trim();
    const description = colonIdx >= 0 ? value.slice(colonIdx + 1).trim() : "";
    if (!seen.has(key)) {
      seen.set(key, description);
    }
  }
  return [...seen.entries()].map(([role, description]) => ({ role, description }));
}
