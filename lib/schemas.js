...
export const tools = [
  {
    name: "calendar",
    description:
      "Manage iCloud calendar events against direct CalDAV server state, not the local macOS EventKit cache. Use calendar names like 'Daily Plan' or 'Shared', or pass a real CalDAV collection URL. Event ids are CalDAV object URLs. Actions: list (calendars), events (query by date range), get (by ID), search (by text), create, update, delete, batch_create, schema (show input schema).",
    inputSchema: {
      type: "object",
      properties: {
        action: {
        ...
        ...agentDXProperties,
        id: { type: "string", description: "Event ID as a CalDAV object URL (get/update/delete)" },
        calendar: { type: "string", description: "Calendar display name (for example 'Daily Plan' or 'Shared'), alias, or CalDAV collection URL" },
        ...
        duration: { type: "number", description: "Duration in minutes (create/update)" },
        ...
        events: {
          ...
          items: {
            ...
            properties: {
              ...
              calendar: { type: "string", description: "Calendar display name, alias, or CalDAV collection URL" },
...

  {
    name: "apple-pim",
    description:
      "PIM system management. Actions: status (check backend/auth state), authorize (request permissions for local domains; calendar just validates CalDAV config), config_show (view config), config_init (discover calendars/lists), schema (show input schema).",
    inputSchema: {
      type: "object",
      properties: {
        action: {
