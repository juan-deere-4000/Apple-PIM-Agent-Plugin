...
function parseChronoComponent(value, refDate = new Date()) {
  if (typeof value !== "string") return null;
  return chrono.parse(value, refDate)[0]?.start || null;
}
...
export function parseUserDate(value, refDate = new Date()) {
  ...
  const parsed = parseChronoComponent(value, refDate);
...
export function parseRangeStart(value, refDate = new Date()) {
  if (value instanceof Date) return value;
  if (typeof value !== "string") return null;
  ...
  if (parsed) {
    ...
    return parsed.isCertain("hour") ? date : startOfDay(date);
  }
...
export function parseRangeEnd(value, refDate = new Date()) {
  if (value instanceof Date) return value;
  if (typeof value !== "string") return null;
  ...
  if (parsed) {
    const date = parsed.date();
    return parsed.isCertain("hour") ? date : addDays(startOfDay(date), 1);
  }
