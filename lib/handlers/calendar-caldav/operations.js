...
export async function getEvent(client, id) {
  ...
  const events = await fetchExpandedOccurrencesForObject(client, calendar, parsedId.objectUrl, range);
...
export function buildSearchRange(args) {
  const now = new Date();
  const start = args.from ? parseRangeStart(args.from) : addDays(now, -30);
  const end = args.to ? parseRangeEnd(args.to, start || now) : addYears(now, 1);
...
async function readOccurrenceAfterSave(client, calendar, calendarObject, occurrenceKey) {
  ...
  const events = await fetchExpandedOccurrencesForObject(client, calendar, calendarObject.url, range);
...
async function fetchExpandedOccurrencesForObject(client, calendar, objectUrl, range) {
  const objects = await client.fetchCalendarObjects({
    calendar,
    timeRange: {
      start: range.start.toISOString(),
  ...
  return objects
    .flatMap((object) => parseCalendarObjectEvents(calendar, object, true))
    .filter((event) => event.objectUrl === objectUrl);
}
