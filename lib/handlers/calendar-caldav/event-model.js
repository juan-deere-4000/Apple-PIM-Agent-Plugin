...
export function buildUpdateVerificationInput(args, savedEvent) {
  const startDate = args.start
    ? ensureParsedDate(args.start, "invalid start date")
    : ensureParsedDate(savedEvent.startDate, "invalid stored start date");
  const endDate = args.end
    ? ensureParsedDate(args.end, "invalid end date")
    : args.duration !== undefined
      ? new Date(startDate.getTime() + Number(args.duration) * 60_000)
      : ensureParsedDate(savedEvent.endDate, "invalid stored end date");
...
export function addExdate(masterComponent, occurrenceKey) {
  const { exdate, tzid } = buildMatchingExdate(masterComponent, occurrenceKey);
  if (!exdate) {
...
function applyEventMutations(component, input) {
  ...
  const existingStartDate = event.startDate.toJSDate();
  const existingEndDate = event.endDate.toJSDate();
  ...
  if (input.allDay !== undefined) {
    nextStartDate ||= existingStartDate;
    nextEndDate ||= existingEndDate;
  }

  if (input.durationMinutes !== undefined) {
    const durationBase = nextStartDate || existingStartDate;
    nextEndDate = new Date(durationBase.getTime() + Number(input.durationMinutes) * 60_000);
  } else if (input.startDate && input.endDate === undefined) {
    const existingDurationMs = existingEndDate.getTime() - existingStartDate.getTime();
    const fallbackDurationMs = existingDurationMs > 0 ? existingDurationMs : 60 * 60_000;
    nextEndDate ||= new Date(nextStartDate.getTime() + fallbackDurationMs);
  }
...
function buildMatchingExdate(masterComponent, occurrenceKey) {
  const occurrence = icalKeyToTime(occurrenceKey);
  if (!occurrence) {
    return { exdate: null, tzid: null };
  }

  const startProperty = masterComponent.getFirstProperty("dtstart");
  const startValue = startProperty?.getFirstValue();
  const tzid = startProperty?.getFirstParameter("TZID") || startProperty?.getFirstParameter("tzid") || null;

  if (!startProperty || !startValue) {
    return { exdate: occurrence, tzid: null };
  }

  if (startValue.isDate) {
    const date = occurrence.toJSDate();
    if (tzid) {
      const parts = getTimeZoneParts(date, tzid);
      return {
        exdate: ICAL.Time.fromData({
          year: parts.year,
          month: parts.month,
          day: parts.day,
          isDate: true,
  ...
  if (tzid) {
    const parts = getTimeZoneParts(occurrence.toJSDate(), tzid);
    return {
      exdate: ICAL.Time.fromData({
        year: parts.year,
        month: parts.month,
