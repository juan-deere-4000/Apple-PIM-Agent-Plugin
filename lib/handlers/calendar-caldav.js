...
async function deleteEvent(client, args) {
  ...
  if (args.futureEvents) {
    throw new Error("deleting future occurrences of a recurring event is not implemented in the caldav backend yet");
  }
...
async function resolveSingleCalendar(client, requestedCalendar, calendarAliases) {
  ...
  const preferred = calendars.find((calendar) => normalize(calendar.displayName) === "daily plan")
    ...
    || calendars[0];
  if (!preferred) {
    throw new Error("no writable icloud calendars found");
  }
  return preferred;
}
...
function applyEventMutations(component, input) {
  ...
  if (input.startDate) {
    event.startDate = toICALTime(input.startDate, input.allDay ?? event.startDate.isDate);
  }
  if (input.endDate) {
    event.endDate = toICALTime(input.endDate, input.allDay ?? event.startDate.isDate);
  }
